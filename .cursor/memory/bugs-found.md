...
| BUG-021 | 🔴 Critical | `library_player_screen.dart` | YouTube plays but loader stuck + play/pause UI wrong — iframe `unknown`/`cued` flipped `_isPlaying` false | User report | Fixed — state machine ignores unknown while playing; single Play/Pause toggle; stronger playback probe |
