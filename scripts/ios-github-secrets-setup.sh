...
echo "STEP 1 — App Store Connect API key (browser only)"
echo "  1. https://appstoreconnect.apple.com/access/integrations/api"
echo "  2. Keys → + → Name: GitHub Actions → Access: Developer"
echo "  3. Download AuthKey_XXXXXX.p8 (once only)"
echo "  4. Note Issuer ID (top of page) and Key ID"
echo ""
echo "  GitHub secrets to add:"
echo "    APP_STORE_CONNECT_ISSUER_ID     = Issuer ID"
echo "    APP_STORE_CONNECT_KEY_ID        = Key ID"
echo "    APP_STORE_CONNECT_API_PRIVATE_KEY = entire .p8 file contents"
echo ""
...
echo "  Then: Actions → Build iOS TestFlight → Run workflow"
echo ""

echo "STEP 6 — TestFlight on iPhone"
echo "  App Store Connect → Radio Udaan → TestFlight → Internal Testing"
echo "  Install Apple TestFlight app → accept invite → install build"
echo ""
